...
class CdMSampler:
  ...
  def run(self, max_iter):
    ...
    etat_init=self.cdm.get_initial_distribution()
    for i in range (max_iter):
    	k=self.draw_from_distribution(etat_init)
    	etat_init=self.cdm.get_transition_distribution(k)
    	self.done=self.notify_receive(i,k)
    	if self.done:
    		break
    self.done=True
